...
export default function Register() {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col items-center justify-center px-4 transition-colors duration-300 py-4 ${dark ? 'bg-gray-950' : 'bg-[#f8f9ff]'}`}
    ...
    >
      ...
      <div className="w-full max-w-sm relative z-10">
        {/* Card */}
        <div
          className={`rounded-2xl p-5 border shadow-sm transition-colors ${dark ? 'bg-gray-900/80 backdrop-blur-sm border-gray-800' : 'bg-white border-gray-200'}`}
        >
          {/* Logo */}
          <Link to="/" className="flex items-center justify-center gap-2 mb-4 group">
            <BotIcon />
            <span className={`text-base font-bold transition ${dark ? 'text-white group-hover:text-blue-400' : 'text-gray-900 group-hover:text-blue-600'}`}>
              BotForge
            </span>
          </Link>

          <h2 className={`text-lg font-bold text-center mb-0.5 ${dark ? 'text-white' : 'text-gray-900'}`}>
            Create account
          </h2>
          <p className={`text-[11px] text-center mb-4 ${dark ? 'text-gray-500' : 'text-gray-400'}`}>
            Start building your AI chatbot
          </p>

          {/* Free plan perks */}
          <div className={`flex items-center justify-center gap-2.5 text-[10px] mb-4 ${dark ? 'text-gray-500' : 'text-gray-500'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-2.5">
            {/* Name */}
            <div>
              <label htmlFor="name" className={`text-[11px] mb-1 block font-medium ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
                Name
              </label>
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full border rounded-lg pl-9 pr-3 py-2 placeholder-gray-400 focus:outline-none focus:ring-1 transition text-sm ${dark ? 'bg-gray-800/80 border-gray-700 text-white focus:border-blue-500 focus:ring-blue-500/30' : 'bg-gray-50 border-gray-200 text-gray-900 focus:border-blue-500 focus:ring-blue-500/30'}`}
                />
              </div>
            </div>
            ...
            {/* Email */}
            <div>
              <label htmlFor="email" className={`text-[11px] mb-1 block font-medium ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
                Email
              </label>
              <div className="relative group">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
            ...
            {/* Password */}
            <div>
              <label htmlFor="password" className={`text-[11px] mb-1 block font-medium ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
                Password
              </label>
              <div className="relative group">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
                ...
                <input
                  ...
                  className={`w-full border rounded-lg pl-9 pr-12 py-2 placeholder-gray-400 focus:outline-none focus:ring-1 transition text-sm ${dark ? 'bg-gray-800/80 border-gray-700 text-white focus:border-blue-500 focus:ring-blue-500/30' : 'bg-gray-50 border-gray-200 text-gray-900 focus:border-blue-500 focus:ring-blue-500/30'}`}
                />
                <button
                  type="button"
                  onClick={() => setShowPass(!showPass)}
                  className={`absolute right-3 top-1/2 -translate-y-1/2 transition text-[10px] ${dark ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'}`}
                >
                  {showPass ? 'Hide' : 'Show'}
                </button>
              </div>
              ...
              {strength && (
                <div className="mt-1.5">
                  <div className={`h-1 rounded-full overflow-hidden ${dark ? 'bg-gray-800' : 'bg-gray-200'}`}>
                    <div className={`h-full rounded-full transition-all ${strength.color} ${strength.width}`} />
                  </div>
                  <p className={`text-[10px] mt-0.5 ${dark ? 'text-gray-500' : 'text-gray-500'}`}>{strength.label}</p>
                </div>
              )}
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2 bg-blue-600 hover:bg-blue-700 rounded-lg font-semibold transition flex items-center justify-center gap-2 disabled:opacity-50 text-sm text-white hover:shadow-[0_0_16px_rgba(37,99,235,0.3)] mt-1"
            >
              {loading ? (
                <>
        ...
        {/* Bottom text */}
        <p className={`text-center text-[10px] mt-3 ${dark ? 'text-gray-600' : 'text-gray-400'}`}>
